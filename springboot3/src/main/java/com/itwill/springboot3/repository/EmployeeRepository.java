...
public interface EmployeeRepository extends JpaRepository<Employee, Integer>{
    ...
    // 대소문자 구분없이 성(lastName)과 이름(firstName)에 문자열이 포함된 직원 찾기:
    List<Employee> findByFirstNameContainingIgnoreCaseOrLastNameContainingIgnoreCase(String firstName, String lastName);

    // JPQL(Java Persistence Query Language)
    // JPA에서 사용하는 "객체지향(object-oriented)" 쿼리 문법.
    // 테이블 이름과 테이블 컬럼 이름으로 쿼리 문장을 작성하는 것이 아니라,
    ...
    List<Employee> findByCity(@Param("city") String cityName);

    // 특정 국가(예: Canada)에 근무하는 직원들 검색
    @Query("select e from Employee e "
            +"where e.department.location.country.countryName = :country")
    List<Employee> findByCountry(@Param("country") String countryName);
}
